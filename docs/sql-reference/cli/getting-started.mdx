...
A client syncing against `http://localhost:8080/db/db1` reads and writes `./dbs/db1/main.db`, which is created on first request unless the server was started with `--readonly`. Database names must match `^[A-Za-z0-9_-]+$` and be at most 128 characters, excluding the Windows reserved device names. See [Command-Line Options](./command-line-options.mdx) for the full details and limitations.
